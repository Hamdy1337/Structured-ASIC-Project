    ...
    try:
        ...
        for tile_name, tile_info in fabric_cells.tiles.items():
            for cell in tile_info.cells:
                rows.append({
                    'tile_name': tile_name,
                    'cell_x': tile_info.x + cell.x,
                    'cell_y': tile_info.y + cell.y,
                    'cell_name': cell.name,
        ...
        result = run_greedy_sa_then_rl_pipeline(
            ...
            max_action_full=512,
            full_placer_train_eps=50,
            swap_refine_train_eps=100,
            batch_size=64,
            device="cpu",
            max_train_batches=30,
            max_apply_batches=50,
            full_steps_per_ep=256,
            swap_steps_per_ep=50,
            enable_timing=True,
            validate_final=True,
            sa_moves_per_temp=5000,
