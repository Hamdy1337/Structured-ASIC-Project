#!/usr/bin/env python3
"""
run_6502_flow.py: Run the complete placement and CTS flow for the 6502 design.
...
def main():
    ...
    # Step 3: Generate CTS Tree Visualization (PNG + HTML)
    print("[Step 3] Generating CTS Tree Visualization...")
    from src.Visualization.cts_plotter import plot_cts_tree_interactive
    
    placement_csv = output_dir / f"{design_name}_placement.csv"
    cts_json = output_dir / f"{design_name}_cts.json"
    cts_output = output_dir / f"{design_name}_cts.html"
    
    if cts_json.exists() and placement_csv.exists():
        plot_cts_tree_interactive(
            placement_csv=str(placement_csv),
            fabric_cells_yaml=str(fabric_cells_path),
            cts_json=str(cts_json),
            output_path=str(cts_output),
            design_name=design_name,
        )
    else:
        print(f"Warning: CTS JSON or placement CSV not found, skipping visualization")
    ...
    print(f"  - {design_name}_cts.html (interactive CTS visualization)")
    print(f"  - {design_name}_cts.png (static CTS image)")
